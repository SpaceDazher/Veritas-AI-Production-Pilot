...
assert.equal(cases.cases.filter((item) => item.status === 'PASS').length, 8);
assert.deepEqual(cases.cases.filter((item) => item.status === 'NOT_PROVEN').map((item) => item.id), ['AC-06', 'AC-09']);
